...
static inline void test_mth_utilities() {
    ...
    Tests.run("is_circular_point_before YES 10 11", is_circular_point_before(10.0, 11.0));
    Tests.run("is_circular_point_before NO 11 10", !is_circular_point_before(11.0, 10.0));
    Tests.run("is_circular_point_before YES 359 0", is_circular_point_before(359.0, 0.0));
    Tests.run("is_circular_point_before NO 0 359", !is_circular_point_before(0.0, 359.0));
    Tests.run("is_circular_point_before NO 179 179", !is_circular_point_before(179.0, 179.0));
    Tests.run("is_circular_point_before YES 179 359", is_circular_point_before(179.0, 359.0));
    Tests.run("is_circular_point_before NO 179 0", !is_circular_point_before(179.0, 0.0));
    Tests.run("is_circular_point_before YES 0 179", is_circular_point_before(0.0, 179.0));
    Tests.run("is_circular_point_before YES 0 180", is_circular_point_before(0.0, 180.0));
    Tests.run("is_circular_point_before YES 180 0", is_circular_point_before(180.0, 0.0));
    Tests.run("is_circular_point_before NO 0 181", !is_circular_point_before(0.0, 181.0));
    Tests.run("is_circular_point_before YES 181 0", is_circular_point_before(181.0, 0.0));
...
static inline void test_str_functions() {
    ...
    Tests.run("is_same_char_string YES 1", is_same_char_string("aaaa"));
    Tests.run("is_same_char_string YES 2", is_same_char_string("  "));
    Tests.run("is_same_char_string YES 3", is_same_char_string("a"));
    Tests.run("is_same_char_string NO", !is_same_char_string("aaba"));
    Tests.run("is_same_char_string NO space", !is_same_char_string("aa a"));
    Tests.run("is_same_char_string NO empty", !is_same_char_string(""));
    Tests.run("is_same_char_string NO null", !is_same_char_string(NULL));
    Tests.run("is_same_char_string_skipping_whitespace YES 1", is_same_char_string_skipping_whitespace("  a aaa"));
    Tests.run("is_same_char_string_skipping_whitespace YES 2", is_same_char_string_skipping_whitespace("a aaa  "));
    Tests.run("is_same_char_string_skipping_whitespace YES 3", is_same_char_string_skipping_whitespace(" a aaa "));
    Tests.run("is_same_char_string_skipping_whitespace NO", !is_same_char_string_skipping_whitespace("  a aba"));
    Tests.run("is_same_char_string_skipping_whitespace NO space", !is_same_char_string_skipping_whitespace("  "));
    Tests.run("is_same_char_string_skipping_whitespace NO empty", !is_same_char_string_skipping_whitespace(""));
    Tests.run("is_same_char_string_skipping_whitespace NO null", !is_same_char_string_skipping_whitespace(NULL));
    ...
    Tests.run("is_uppercase_string NULL", !is_uppercase_string(NULL));
    Tests.run("is_uppercase_string space", is_uppercase_string(" "));
    Tests.run("is_uppercase_string a a", !is_uppercase_string("a a"));
    Tests.run("is_uppercase_string A A", is_uppercase_string("A A"));
    Tests.run("is_lowercase_string NULL", !is_lowercase_string(NULL));
    Tests.run("is_lowercase_string space", is_lowercase_string(" "));
    Tests.run("is_lowercase_string A A", !is_lowercase_string("A A"));
    Tests.run("is_lowercase_string a a", is_lowercase_string("a a"));
    ...
    Tests.run("bool_to_int_char 0", bool_to_int_char(false) == '0');
    Tests.run("bool_to_int_char 1", bool_to_int_char(true) == '1');
    ...
    Tests.run("match_starting_pattern text pattern 1", match_starting_pattern("text", "t?x", '?'));
    Tests.run("match_starting_pattern text pattern 2", match_starting_pattern("text", "t??t", '?'));
    Tests.run("match_starting_pattern text pattern 3", match_starting_pattern("text", "t?xt", '?'));
    Tests.run("match_starting_pattern text pattern 4", match_starting_pattern("text", "?ext", '?'));
    Tests.run("match_starting_pattern text pattern 5", match_starting_pattern("text", "tex?", '?'));
    Tests.run("match_starting_pattern text pattern 6", match_starting_pattern("text", "????", '?'));
    Tests.run("match_starting_pattern text pattern 7", match_starting_pattern("text", "???", '?'));
    Tests.run("match_starting_pattern text long", !match_starting_pattern("text", "t?xts", '?'));
    Tests.run("match_starting_pattern empty text", !match_starting_pattern("", "text", '?'));
    Tests.run("match_starting_pattern text empty", match_starting_pattern("text", "", '?'));
    Tests.run("match_starting_pattern NULL text", !match_starting_pattern(NULL, "text", '?'));
    Tests.run("match_starting_pattern text NULL", !match_starting_pattern("text", NULL, '?'));
    Tests.run("match_starting_pattern NULL NULL", !match_starting_pattern(NULL, NULL, '?'));
    Tests.run("match_starting_pattern no wildard 1", match_starting_pattern("abc", "abc", CHARS_NULL));
    Tests.run("match_starting_pattern no wildard 2", !match_starting_pattern("abc", "non", CHARS_NULL));
    Tests.run("match_ending_pattern text pattern 1", match_ending_pattern("text", "e?t", '?'));
    Tests.run("match_ending_pattern text pattern 2", match_ending_pattern("text", "t??t", '?'));
    Tests.run("match_ending_pattern text pattern 3", match_ending_pattern("text", "te?t", '?'));
    Tests.run("match_ending_pattern text pattern 4", match_ending_pattern("text", "tex?", '?'));
    Tests.run("match_ending_pattern text pattern 5", match_ending_pattern("text", "?ext", '?'));
    Tests.run("match_ending_pattern text pattern 6", match_ending_pattern("text", "????", '?'));
    Tests.run("match_ending_pattern text pattern 7", match_ending_pattern("text", "???", '?'));
    Tests.run("match_ending_pattern text long", !match_ending_pattern("text", "at?xt", '?'));
    Tests.run("match_ending_pattern empty text", !match_ending_pattern("", "text", '?'));
    Tests.run("match_ending_pattern text empty", match_ending_pattern("text", "", '?'));
    Tests.run("match_ending_pattern NULL text", !match_ending_pattern(NULL, "text", '?'));
    Tests.run("match_ending_pattern text NULL", !match_ending_pattern("text", NULL, '?'));
    Tests.run("match_ending_pattern NULL NULL", !match_ending_pattern(NULL, NULL, '?'));
    Tests.run("match_ending_pattern no wildard 1", match_ending_pattern("abc", "abc", CHARS_NULL));
    Tests.run("match_ending_pattern no wildard 2", !match_ending_pattern("abc", "non", CHARS_NULL));
    tmp = trim_left("  abc");
    ...
    string_key_value_destroy(kv);
    kv = string_key_value_parse("username", sep);
    Tests.run("string_key_value_parse username=", assigned(kv) && streq(kv->key, "username") && kv->value == NULL);
    string_key_value_destroy(kv);
    kv = string_key_value_parse("=", sep);
    Tests.run("string_key_value_parse =", assigned(kv) && !has_content(kv->key) && !has_content(kv->value));
    string_key_value_destroy(kv);
    kv = string_key_value_parse("", sep);
    Tests.run("string_key_value_parse empty", kv == NULL);
    kv = string_key_value_parse(NULL, sep);
    Tests.run("string_key_value_parse null", kv == NULL);
    kv = string_key_value_parse("username=admin", "=");
    char* kvs = string_key_value_write(kv, "::");
    Tests.run("string_key_value_write good", streq(kvs, "username::admin"));
    free(kvs);
    kvs = string_key_value_write(NULL, STRINGS_NOTHING);
    Tests.run("string_key_value_write null 1", kvs == NULL);
